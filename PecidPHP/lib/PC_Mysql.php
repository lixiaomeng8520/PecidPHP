...
class PC_Mysql extends PC_Lib{
	...
	private function _init_db(){
		$this->_link = @mysql_connect($this->_config['host'].':'.$this->_config['port'], $this->_config['user'], $this->_config['pass']);
		if(!$this->_link){
			trigger_error('conn error: '.mysql_error(), E_USER_ERROR);
		}
		mysql_query('set names '.$this->_config['char'], $this->_link);
		mysql_select_db($this->_config['dbnm'], $this->_link);
	}
...
/*					case 'i':
	...
	private function _parseTable($table){
		...
		return $this->_config['pref'].$table;
	}
